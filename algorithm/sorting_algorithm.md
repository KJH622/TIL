...
- 일반적으로 문제 상황에 따라서 적절한 정렬 알고리즘이 공식처럼 사용된다.
    ...
    ![정렬 알고리즘](/algorithm/img/sorting/sorting_1.png)
...
for i in range(len(array)):
    # i는 가장 작은 데이터와 위치가 바뀔 인덱스를 의미
    min_index = i # 가장 작은 원소의 인덱스
    for j in range(i + 1, len(array)):
        if array[min_index] > array[j]:
            min_index = j
    # 가장 앞쪽 원소와 가장 작은 원소의 위치를 변경
...
- 구현 방식에 따라서 사소한 오차는 있을 수 있지만, 전체 연산 횟수는 다음과 같다.
    ...
    ![선택 정렬](/algorithm/img/sorting/sorting_8.png)
...
- **[Step 0]** 첫 번째 데이터 '7'은 그 자체로 정렬이 되어 있다고 판단하고, 두 번째 데이터인 '5'가 어떤 위치로 들어갈지 판단한다. '7'의 왼쪽으로 들어가거나 오른쪽으로 들어가거나 두 경우만 존재한다.
    ...
    ![삽입 정렬](/algorithm/img/sorting/insertion_sort_1.png)

- **[Step 1]** 이어서 '9'가 어떤 위치로 들어갈지 판단한다.

    ![삽입 정렬](/algorithm/img/sorting/insertion_sort_2.png)

- **[Step 2]** 이어서 '0'이 어떤 위치로 들어갈지 판단한다.

    ![삽입 정렬](/algorithm/img/sorting/insertion_sort_3.png)

- **[Step 3]** 이어서 '3'이 어떤 위치로 들어갈지 판단한다.

    ![삽입 정렬](/algorithm/img/sorting/insertion_sort_4.png)

- 이러한 과정을 반복하면 다음과 같이 정렬이 완료된다.

    ![삽입 정렬](/algorithm/img/sorting/insertion_sort_5.png)
...
- **[Step 2]** 현재 피벗의 값은 '5'이다. 왼쪽에서부터 '5'보다 큰 데이터를 선택하므로 '6'이 선택되고, 오른쪽에서부터 '5'보다 작은 데이터를 선택하므로 '1'이 선택된다. 단, 이처럼 **위치가 엇갈리는 경우 '피벗'과 '작은 데이터'의 위치를 서로 변경한다.**
    ...
    ![퀵 정렬](/algorithm/img/sorting/quick_sort_3.png)

- **[분할 완료]** 이제 '5'의 왼쪽에 있는 데이터는 모두 5보다 작고, 오른쪽에 있는 데이터는 모두 '5'보다 크다는 특징이 있다. 이렇게 **피벗을 기준으로 데이터 묶음을 나누는 작업** 을 **분할(Divide)** 이라고 한다.

    ![퀵 정렬](/algorithm/img/sorting/quick_sort_4.png)

- **[왼쪽 데이터 묶음 정렬]** 왼쪽에 있는 데이터에 대해서 마찬가지로 정렬을 수행한다.

    ![퀵 정렬](/algorithm/img/sorting/quick_sort_5.png)

- **[오른쪽 데이터 묶음 정렬]** 오른쪽에 있는 데이터에 대해서 마찬가지로 정렬을 수행한다.

    - 이러한 과정을 반복하면 전체 데이터에 대해서 정렬이 수행된다.

    ![퀵 정렬](/algorithm/img/sorting/quick_sort_6.png)

### 퀵 정렬이 빠른 이유: 직관적인 이해

- 이상적인 경우 분할이 절반씩 일어나면 전체 연산 횟수로 O(NlogN)를 기대할 수 있다.

    - **너비 * 높이 = N * logN = NlogN**

    ![퀵 정렬](/algorithm/img/sorting/quick_sort_7.png)
...
- 하지만 최악의 경우 **O(N<sup>2</sup>)** 의 시간 복잡도를 가진다.
    ...
    - 그 경우는 이미 정렬된 배열의 경우를 의미한다. 왜냐하면 분할이 이루어질 때마다 오른쪽 데이터만 남기 때문이다.

        => 분할을 하기 위해서 매번 선형 탐색을 해야 한다.

    - 첫 번째 원소를 피벗을 삼을 때, 이미 정렬된 배열에 대해서 퀵 정렬을 수행하면 어떻게 될까?

    ![퀵 정렬](/algorithm/img/sorting/quick_sort_8.png)
...
- **[Step 0]** 가장 작은 데이터부터 가장 큰 데이터까지의 범위가 모두 담길 수 있도록 리스트를 생성한다.
    ...
    ![계수 정렬](/algorithm/img/sorting/counting_sort_1.png)

- **[Step 1]** 데이터를 하나씩 확인하며 데이터의 값과 동일한 인덱스의 데이터를 1씩 증가시킨다.

    - **정렬할 데이터** : **<span style="color:red">7</span>** 5 9 0 3 1 6 2 9 1 4 8 0 5 2

    ![계수 정렬](/algorithm/img/sorting/counting_sort_2.png)

- **[Step 2]** 데이터를 하나씩 확인하며 데이터의 값과 동일한 인덱스의 데이터를 1씩 증가시킨다.

    - **정렬할 데이터** : **<span style="color:red">7 5</span>** 9 0 3 1 6 2 9 1 4 8 0 5 2

    ![계수 정렬](/algorithm/img/sorting/counting_sort_3.png)

- **[Step 3]** 데이터를 하나씩 확인하며 데이터의 값과 동일한 인덱스의 데이터를 1씩 증가시킨다.

    - **정렬할 데이터** : **<span style="color:red">7 5 9</span>** 0 3 1 6 2 9 1 4 8 0 5 2

    ![계수 정렬](/algorithm/img/sorting/counting_sort_4.png)

- **[Step 15]** 결과적으로 최종 리스트에는 각 데이터가 몇 번씩 등장했는지 그 횟수가 기록된다.

    - **정렬할 데이터** : **<span style="color:red">7 5 9 0 3 1 6 2 9 1 4 8 0 5 2</span>**

    ![계수 정렬](/algorithm/img/sorting/counting_sort_5.png)

- **결과를 확인할 때**는 리스트의 첫 번째 데이터부터 하나씩 그 값만큼 반복하여 인덱스를 출력한다.

    ![계수 정렬](/algorithm/img/sorting/counting_sort_6.png)
...
count = [0] * (max(array) + 1) # 0 ~ 9까지 총 10개
...
- 계수 정렬은 때에 따라서 심각한 비효율성을 초래할 수 있다.
    ...
    - 즉, 데이터의 범위가 너무 크다면 계수 정렬을 사용하기 어렵다.
...
![정렬 알고리즘](/algorithm/img/sorting/sorting_9.png)
...
- N, K, 그리고 배열 A와 B의 정보가 주어졌을 때, 최대 K번의 바꿔치기 연산을 수행하여 만들 수 있는 **배열 A의 모든 원소의 합의 최댓값을 출력하는 프로그램**을 작성하시오.
...
![정렬 알고리즘](/algorithm/img/sorting/sorting_10.png)

### 문제 해결 아이디어

- **핵심 아이디어 : 매번 배열 A에서 가장 작은 원소를 골라서, 배열 B에서 가장 큰 원소와 교체**한다.

- 가장 먼저 배열 A와 배열 B가 주어지면 A에 대하여 오름차순 정렬하고, B에 대하여 내림차순 정렬한다.
