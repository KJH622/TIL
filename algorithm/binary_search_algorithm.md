...
- 이미 정렬된 10개의 데이터 중에서 값이 4인 원소를 찾는 예시를 살펴보자.
    ...
    ![이진 탐색](/algorithm/img/binary_search/binary_search_1.png)

- **[Step 1]** 시작점: 0, 끝점: 9, 중간점: 4 (소수점 이하 제거)

    - 시작점, 끝점, 중간점은 모두 인덱스를 의미한다.

    - `(0 + 9) / 2 = 4.5`이다. 그러나 소수점 이하는 제거하여 중간점은 4이다.

    ![이진 탐색](/algorithm/img/binary_search/binary_search_2.png)

- **[Step 2]** 시작점: 0, 끝점: 3, 중간점: 1 (소수점 이하 제거)

    - 찾고자 하는 숫자가 중간점 보다 작으면 중간점을 포함하여 오른쪽의 값들은 신경쓰지 않는다.

    ![이진 탐색](/algorithm/img/binary_search/binary_search_3.png)

- **[Step 3]** 시작점: 2, 끝점: 3, 중간점: 2 (소수점 이하 제거)

    - 찾고자 하는 숫자가 중간점보다 크기 때문에 중간점을 포함하여 왼쪽의 값들은 신경쓰지 않는다.

    ![이진 탐색](/algorithm/img/binary_search/binary_search_4.png)

    - 중간점과 찾고자 하는 숫자가 동일하기 때문에 탐색을 멈춘다.
...
- **`bisect_right(a, x)`** : 정렬된 순서를 유지하면서 배열 a에 x를 삽입할 가장 오른쪽 인덱스를 반환
    ...
    ![이진 탐색 라이브러리](/algorithm/img/binary_search/binary_search_5.png)
...
- **파라메트릭 서치**란 **최적화 문제를 결정 문제('예' 혹은 '아니오')로 바꾸어 해결하는 기법**이다.
...
![떡볶이 떡 만들기](/algorithm/img/binary_search/binary_search_6.png)
...
- **[Step 1]** 시작점: 0, 끝점: 19, 중간점: 9
    ...
    ![떡볶이 떡 만들기](/algorithm/img/binary_search/binary_search_7.png)

- **[Step 2]** 시작점: 10, 끝점: 19, 중간점: 14

    - **이때 필요한 떡의 크기: M = 6이므로, 결과 저장**

    ![떡볶이 떡 만들기](/algorithm/img/binary_search/binary_search_8.png)

- **[Step 3]** 시작점: 15, 끝점: 19, 중간점: 17

    - **이때 필요한 떡의 크기: M = 6이므로, 결과 저장하지 않음**

    ![떡볶이 떡 만들기](/algorithm/img/binary_search/binary_search_9.png)

- **[Step 4]** 시작점: 15, 끝점: 16, 중간점: 15

    - **이때 필요한 떡의 크기: M = 6이므로, 결과 저장**

    ![떡볶이 떡 만들기](/algorithm/img/binary_search/binary_search_10.png)
...
start = 0
end = max(array) # 가지고 있는 떡 중 가장 긴 떡의 길이
...
- N개의 원소를 포함하고 있는 수열이 오름차순으로 정렬되어 있다. 이때 이 **수열에서 x가 등장한 횟수를 계산**하시오. 예를 들어 수열 {1, 1, 2, 2, 2, 2, 3}이 있을 때 x = 2라면, 현재 수열에서 값이 2인 원소가 4개이므로 4를 출력한다.

- 단, 이 문제는 시간 복잡도 **O(logN)** 으로 알고리즘을 설꼐하지 않으면 **시간 초과** 판정을 받는다.

### 문제 조건

![떡볶이 떡 만들기](/algorithm/img/binary_search/binary_search_11.png)
...
- 특정 값이 등장하는 첫 번째 위치와 마지막 위치를 찾아 위치 차이를 계산해 문제를 해결할 수 있다.
    ...
    ![이진 탐색](/algorithm/img/binary_search/binary_search_12.png)
