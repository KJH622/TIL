...
- 서로소 집합(Disjoint Sets)란 **공통 원소가 없는 두 집합**을 의미한다.
    ...
    ![서로소 집합](/algorithm/img/other_graph_theory/disjoint_sets_1.png)
...
- 합치기 연산할 때는 더 큰 루트 노드가 더 작은 루트 노드를 가리키도록 하는 것이 관행이다.
...
- **[초기 단계]** 노드의 개수 크기의 부모 테이블을 초기화한다.
    ...
    ![서로소 집합](/algorithm/img/other_graph_theory/disjoint_sets_2.png)

- **처리할 연산들 : Union(1, 4)**, Union(2, 3), Union(2, 4), Union(5, 6)

- **[Step 1]** 노드 1과 노드 4의 루트 노드를 각각 찾는다. 현재 루트 노드는 각각 1과 4이므로 더 큰 번호에 해당하는 루트 노드 4의 부모를 1로 설정한다.

    ![서로소 집합](/algorithm/img/other_graph_theory/disjoint_sets_3.png)

- **처리할 연산들 : Union(1, 4), Union(2, 3)**, Union(2, 4), Union(5, 6)

- **[Step 2]** 노드 2와 노드 3의 루트 노드를 각각 찾는다. 현재 루트 노드는 각각 2와 3이므로 더 큰 번호에 해당하는 루트 노드 3의 부모를 2로 설정한다.

    ![서로소 집합](/algorithm/img/other_graph_theory/disjoint_sets_4.png)

- **처리할 연산들 : Union(1, 4), Union(2, 3), Union(2, 4)**, Union(5, 6)

- **[Step 3]** 노드 2와 노드 4의 루트 노드를 각각 찾는다. 현재 루트 노드는 각각 2와 1이므로 더 큰 번호에 해당하는 루트 노드 2의 부모를 1로 설정한다.

    - 노드 3의 루트 노드는 2이고 노드 2의 루트 노드는 1이므로 노드 3과 노드 4의 루트 노드는 1로서 동일하다.

    - 즉, 노드 3과 노드 4는 같은 집합 내에 존재함을 알 수 있다.

    ![서로소 집합](/algorithm/img/other_graph_theory/disjoint_sets_5.png)

- **처리할 연산들 : Union(1, 4), Union(2, 3), Union(2, 4), Union(5, 6)**

- **[Step 4]** 노드 5과 노드 6의 루트 노드를 각각 찾는다. 현재 루트 노드는 각각 5와 6이므로 더 큰 번호에 해당하는 루트 노드 6의 부모를 5로 설정한다.

    ![서로소 집합](/algorithm/img/other_graph_theory/disjoint_sets_6.png)

### 서로소 집합 자료구조: 연결성

- 서로소 집합 자료구조에서는 **연결성**을 통해 손쉽게 집합의 형태를 확인할 수 있다.

    ![서로소 집합](/algorithm/img/other_graph_theory/disjoint_sets_7.png)
...
- 다음 예시에서 노드 3의 루트를 찾기 위해서는 노드 2를 거쳐 노드 1에 접근해야 한다.
    ...
    ![서로소 집합](/algorithm/img/other_graph_theory/disjoint_sets_8.png)
...
def find_parent(parent, x):
    # 루트 노드를 찾을 때까지 재귀 호출
    # 부모 노드가 루트 노드(자기 자신)가 아니라면
    if parent[x] != x:
        return find_parent(parent, parent[x])
    return x # 특정 원소가 속한 루트 번호
...
- 최악의 경우에는 찾기(Find) 함수가 모든 노드를 다 확인하게 되어 시간 복잡도가 **O(V)** 이다.
    ...
    - **수행된 연산들** : Union(4, 5), Union(3, 4), Union(2, 3), Union(1, 2)
        ...
        - 1번 노드부터 5번 노드까지 같은 집합에 속하며 선형적으로 연결되어 있는 경우를 의미한다.

        ![서로소 집합](/algorithm/img/other_graph_theory/disjoint_sets_9.png)

### 서로소 집합 자료구조: 경로 압축

- 찾기(Find) 함수를 최적화하기 위한 방법으로 경로 압축(Path Compression)을 이용할 수 있다.

- 경로 압축: 루트 노드까지 도달하기 위한 경로를 압축해서 시간을 단축할 수 있다.
...
- 기본적인 방법에 비하여 시간 복잡도가 개선된다.
    ...
    ![서로소 집합](/algorithm/img/other_graph_theory/disjoint_sets_10.png)
...
- **사이클 판별 알고리즘**은 다음과 같다.
    ...
    1. 각 간선을 하나씩 확인하며 두 노드의 루트 노드를 확인한다.
        ...
        1. 루트 노드가 서로 다르다면(다른 집합) 두 노드에 대하여 합집합(Union) 연산을 수행한다.
...
- **[초기 단계]** 모든 노드에 대하여 자기 자신을 부모로 설정하는 형태로 부모 테이블을 초기화한다.
    ...
    ![서로소 집합](/algorithm/img/other_graph_theory/disjoint_sets_11.png)

- **[Step 1]** 간선 (1, 2)를 확인한다. 노드 1과 노드 2의 루트 노드는 각각 1과 2이다. 따라서 더 큰 번호에 해당하는 노드 2의 부모 노드를 1로 변경한다.

    ![서로소 집합](/algorithm/img/other_graph_theory/disjoint_sets_12.png)

- **[Step 2]** 간선 (1, 3)을 확인한다. 노드 1과 노드 3의 루트 노드는 각각 1과 3이다. 따라서 더 큰 번호에 해당하는 노드 3의 부모 노드를 1로 변경한다.

    ![서로소 집합](/algorithm/img/other_graph_theory/disjoint_sets_13.png)

- **[Step 3]** 간선 (2, 3)을 확인한다. 이미 노드 2와 노드 3의 루트 노드는 모두 1이다. 다시 말해 **사이클이 발생**한다는 것을 알 수 있다.

    ![서로소 집합](/algorithm/img/other_graph_theory/disjoint_sets_14.png)
...
- **그래프에서 모든 노드를 포함하면서 사이클이 존재하지 않는 부분 그래프**를 의미한다.
    ...
    - 모든 간선을 포함하지는 않고 일부 간선으로도 모든 노드가 연결된다.

    - 사이클이 발생하면 안된다.

    - 모든 노드가 포함되어 서로 연결되면서 사이클이 존재하지 않는다는 조건은 **트리**의 조건이기도 하다.

    ![크루스칼 알고리즘](/algorithm/img/other_graph_theory/kruskal_algorithm_1.png)
...
- 예를 들어 N개의 도시가 존재하는 상황에서 두 도시 사이에 도로를 놓아 **전체 도시가 서로 연결**될 수 있게 도로를 설치하는 경우를 생각해보자.
    ...
    ![크루스칼 알고리즘](/algorithm/img/other_graph_theory/kruskal_algorithm_2.png)
...
- 최소 신장 트리의 포함되어 있는 간선의 개수 :  **전체 노드 개수 -1**
...
- **[초기 단계]** 그래프의 모든 간선 정보에 대하여 **오름차순 정렬을 수행**한다.
    ...
    ![크루스칼 알고리즘](/algorithm/img/other_graph_theory/kruskal_algorithm_3.png)

- **[Step 1]** 아직 처리하지 않은 간선 중에서 가장 짧은 간선인 (3, 4)를 선택하여 처리한다.

    ![크루스칼 알고리즘](/algorithm/img/other_graph_theory/kruskal_algorithm_4.png)

- **[Step 2]** 아직 처리하지 않은 간선 중에서 가장 짧은 간선인 (4, 7)을 선택하여 처리한다.

    ![크루스칼 알고리즘](/algorithm/img/other_graph_theory/kruskal_algorithm_5.png)

- **[Step 3]** 아직 처리하지 않은 간선 중에서 가장 짧은 간선인 (4, 6)을 선택하여 처리한다.

    ![크루스칼 알고리즘](/algorithm/img/other_graph_theory/kruskal_algorithm_6.png)

- **[Step 4]** 아직 처리하지 않은 간선 중에서 가장 짧은 간선인 (6, 7)을 선택하여 처리한다.

    ![크루스칼 알고리즘](/algorithm/img/other_graph_theory/kruskal_algorithm_7.png)

- **[Step 5]** 아직 처리하지 않은 간선 중에서 가장 짧은 간선인 (1, 2)을 선택하여 처리한다.

    ![크루스칼 알고리즘](/algorithm/img/other_graph_theory/kruskal_algorithm_8.png)

- **[Step 6]** 아직 처리하지 않은 간선 중에서 가장 짧은 간선인 (2, 6)을 선택하여 처리한다.

    ![크루스칼 알고리즘](/algorithm/img/other_graph_theory/kruskal_algorithm_9.png)

- **[Step 7]** 아직 처리하지 않은 간선 중에서 가장 짧은 간선인 (2, 3)을 선택하여 처리한다.

    ![크루스칼 알고리즘](/algorithm/img/other_graph_theory/kruskal_algorithm_10.png)

- **[Step 8]** 아직 처리하지 않은 간선 중에서 가장 짧은 간선인 (5, 6)을 선택하여 처리한다.

    ![크루스칼 알고리즘](/algorithm/img/other_graph_theory/kruskal_algorithm_11.png)

- **[Step 9]** 아직 처리하지 않은 간선 중에서 가장 짧은 간선인 (1, 5)을 선택하여 처리한다.

    ![크루스칼 알고리즘](/algorithm/img/other_graph_theory/kruskal_algorithm_12.png)

- **[알고리즘 수행 결과]**

    - 최소 신장 트리에 포함되어 있는 간선의 비용만 모두 더하면, 그 값이 최종 비용에 해당한다.

    ![크루스칼 알고리즘](/algorithm/img/other_graph_theory/kruskal_algorithm_13.png)
...
- 예시) 선수과목을 고려한 학습 순서 설정
    ...
    ![위상 정렬](/algorithm/img/other_graph_theory/status_range_1.png)

- 위 세 과목을 모두 듣기 위한 **적절한 학습 순서**는?

    - 자료구조 -> 알고리즘 -> 고급 알고리즘 **<span style="color:blue">(O)</span>**

    - 자료구조 -> 고급 알고리즘 -> 알고리즘 **<span style="color:red">(X)</red>**
...
- **진출차수(Outdegree)** : 특정한 노드에서 나가는 간선의 개수
    ...
    ![위상 정렬](/algorithm/img/other_graph_theory/status_range_2.png)
...
- 위상 정렬을 수행할 그래프를 준비한다.
    ...
    ![위상 정렬](/algorithm/img/other_graph_theory/status_range_3.png)

- **[초기 단계]** 초기 단계에서는 **진입차수가 0인 모든 노드**를 큐에 넣는다.

    - 처음에 **노드 1**이 큐에 삽입된다.

    ![위상 정렬](/algorithm/img/other_graph_theory/status_range_4.png)

- **[Step 1]** 큐에서 **노드 1**을 꺼낸 뒤에 **노드 1**에서 나가는 간선을 제거한다.

    - 새롭게 **진입차수가 0이 된 노드**들을 큐에 삽입한다.

    - 한 번의 스텝에서 여러 개의 노드가 들어갈 경우 아무 노드를 먼저 꺼내도 상관은 없지만 더 작은 노드를 먼저 꺼낸다고 가정한다.

    ![위상 정렬](/algorithm/img/other_graph_theory/status_range_5.png)

- **[Step 2]** 큐에서 **노드 2**를 꺼낸 뒤에 **노드 2**에서 나가는 간선을 제거한다.

    - 새롭게 **진입차수가 0이 된 노드**를 큐에 삽입한다.

    ![위상 정렬](/algorithm/img/other_graph_theory/status_range_6.png)

- **[Step 3]** 큐에서 **노드 5**를 꺼낸 뒤에 **노드 5**에서 나가는 간선을 제거한다.

    - 새롭게 **진입차수가 0이 된 노드**를 큐에 삽입한다.

    ![위상 정렬](/algorithm/img/other_graph_theory/status_range_7.png)

- **[Step 4]** 큐에서 **노드 3**를 꺼낸 뒤에 **노드 3**에서 나가는 간선을 제거한다.

    - 새롭게 **진입차수가 0이 된 노드**가 없으므로 그냥 넘어간다.

    ![위상 정렬](/algorithm/img/other_graph_theory/status_range_8.png)

- **[Step 5]** 큐에서 **노드 6**를 꺼낸 뒤에 **노드 6**에서 나가는 간선을 제거한다.

    - 새롭게 **진입차수가 0이 된 노드**를 큐에 삽입한다.

    ![위상 정렬](/algorithm/img/other_graph_theory/status_range_9.png)

- **[Step 6]** 큐에서 **노드 4**를 꺼낸 뒤에 **노드 4**에서 나가는 간선을 제거한다.

    - 새롭게 **진입차수가 0이 된 노드**를 큐에 삽입한다.

    ![위상 정렬](/algorithm/img/other_graph_theory/status_range_10.png)

- **[위상 정렬 결과]**

    - 큐에 삽입된 전체 노드 순서: **1 -> 2 -> 5 -> 3 -> 6 -> 4 -> 7**

    ![위상 정렬](/algorithm/img/other_graph_theory/status_range_11.png)
